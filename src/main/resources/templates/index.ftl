            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="text-align:center;">[<a
                            href="/countries/view/${country.id?c}">修改</a>] -
                        [<a href="/countries/delete/${country.id?c}">删除</a>]
                    </td>
                </tr>
                </#list>
            </tbody>
        ...
        <table class="gridtable" style="width:100%;text-align: center;">
            <tr>
                ...
                <#list pageInfo.navigatepageNums as nav>
                    ...
                    <#if nav != pageInfo.pageNum>
                        <td>
                            <a href="/countries?page=${nav}&rows=${pageInfo.pageSize}&countryname=<#if queryParam.countryname??>${queryParam.countryname}</#if>&countrycode=<#if queryParam.countrycode??>${queryParam.countrycode}</#if>">${nav}</a>
                        </td>
                    </#if>
                </#list>
                <#if pageInfo.hasNextPage>
                    <td>
                        <a href="/countries?page=${pageInfo.nextPage}&rows=${pageInfo.pageSize}&countryname=<#if queryParam.countryname??>${queryParam.countryname}</#if>&countrycode=<#if queryParam.countrycode??>${queryParam.countrycode}</#if>">下一页</a>
                    </td>
                    <td>
                        <a href="/countries?page=${pageInfo.pages}&rows=${pageInfo.pageSize}&countryname=<#if queryParam.countryname??>${queryParam.countryname}</#if>&countrycode=<#if queryParam.countrycode??>${queryParam.countrycode}</#if>">尾页</a>
                    </td>
                </#if>
            </tr>
        </table>
